'''
this pattern opens up various possibility of processing a given request.
this pattern decouple the request and its processing.
solution:
    Abstractor handler: that stores a successor who will handle a request if it is not gandled at the current handler
    concrete handler  : check if they can handle the request, if they can, they handle and return true value indicating
                        that they have handled the request
